...
for (const status of ["checkmated", "stalemated"] as const) {
  test(`${status}: retained army is grey, unselectable, unchecked and zero-point capturable`, async ({ page }) => {
    ...
    const initial: GameState = { ...base, board, turn:PlayerColor.Green, positionCounts:{},
    ...
    await expect(page.getByText(`Red is ${status}`, { exact:false })).toContainText("zero points");
