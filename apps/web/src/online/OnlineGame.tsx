...
function ConnectedGame({room,session,onLeave}:{room:string;session:Session;onLeave:()=>void}){
  ...
  const moves=state&&view.phase==="connected"&&view.control?.controller&&!view.pending&&state.turn===seat?[...legalMoves(state)]:[];
  ...
  const leave=async()=>{if((view.pending||view.identityConflict)&&!window.confirm("A sent command may already be committed. Leave and clear its saved intention after abandoning receipt recovery?"))return;await client.leave();onLeave();};
  ...
  return <main className="game-shell"><header className="app-header"><h1>Private multiplayer</h1><button onClick={()=>void leave()}>Leave online room</button></header>
    ...
    {state&&snapshot&&<><p data-testid="online-boundary">Command {snapshot.command} · {snapshot.stateHash}</p>
      ...
      <div className="board-tools"><button disabled={!view.control?.controller||view.phase!=="connected"||!!view.pending||seat===undefined||state.players[seat].status!=="active"} onClick={()=>{if(window.confirm("Resign your seat? During the opening this aborts the game."))void client.command({type:"resign"});}}>Resign my seat</button>
        {seat!==undefined&&canClaimWin(state,seat)&&<button disabled={!view.control?.controller||view.phase!=="connected"||!!view.pending} onClick={()=>{if(window.confirm("Claim Win now and end this game?"))void client.command({type:"claimWin"});}}>Claim Win</button>}</div>
